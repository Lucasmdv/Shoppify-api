...
public record StoreRequest(
                @Schema(description = "Name of the store", example = "Main Store", minLength = 3, maxLength = 15)
                @NotBlank(message = "Store name is required")
                @Size(min = 3, max = 15, message = "Store name must be between 3 and 15 characters")
                String storeName,

                @Schema(description = "Store address", example = "123 Main St", maxLength = 100)
                @NotBlank(message = "Address is required")
                @Size(max = 100, message = "Address must be less than or equal to 100 characters")
                String address,

                @Schema(description = "City where the store is located", example = "New York", minLength = 3, maxLength = 100)
                @NotBlank(message = "City is required")
                @Size(min = 3, max = 100, message = "City must be between 3 and 100 characters")
                String city,

                @Schema(description = "Phone of the store", example = "223-4817825", minLength = 3, maxLength = 20)
                @NotBlank(message = "Phone is required.")
                @Size(min = 3, max = 20, message = "Phone must be between 3 and 20 characters")
                String phone,

                @Schema(description = "Postal Code", example = "1900", maxLength = 20)
                @NotBlank(message = "Postal code is required")
                @Size(max = 20, message = "Postal code must be less than or equal to 20 characters")
                String postalCode,

                @Schema(description = "Facebook page URL", example = "https://facebook.com/shoppify", minLength = 3, maxLength = 100)
                @Size(min = 3, max = 100, message = "Facebook URL must be between 3 and 100 characters") String facebook,

                @Schema(description = "Instagram profile URL", example = "https://instagram.com/shoppify", minLength = 3, maxLength = 100)
                @Size(min = 3, max = 100, message = "Instagram URL must be between 3 and 100 characters") String instagram,

                @Schema(description = "X (Twitter) profile URL", example = "https://x.com/shoppify", minLength = 3, maxLength = 100)
                ...
                @Valid List<HomeCarouselItem> homeCarousel,
